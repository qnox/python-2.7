#!/usr/bin/env bash
# Build libffi from source with ARM64 patches from python-build-standalone
...
# Detect platform and set CPU count
if [[ "$OSTYPE" == "darwin"* ]]; then
    NUM_CPUS=$(sysctl -n hw.ncpu)
    APPLE_MIN_DEPLOYMENT_TARGET="${MACOSX_DEPLOYMENT_TARGET:-11.0}"
...
INSTALL_DIR="${INSTALL_PREFIX:-${DEPS_DIR}/libffi}"
...
if [ ! -f "libffi-${LIBFFI_VERSION}.tar.gz" ]; then
    ...
    curl -L -o "libffi-${LIBFFI_VERSION}.tar.gz" \
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    # Match Python's deployment target to avoid ABI incompatibilities
    ARCH_FLAGS="${ARCH_FLAGS} -mmacosx-version-min=${APPLE_MIN_DEPLOYMENT_TARGET}"
fi

EXTRA_CONFIGURE=""
# mkostemp() was introduced in macOS 10.10
